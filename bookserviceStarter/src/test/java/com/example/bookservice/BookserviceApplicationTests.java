...
public class BookserviceApplicationTests {
	...
	public void testDeleteBook() throws Exception{
		mvc.perform(
				MockMvcRequestBuilders.get("/book/7")
		)
				.andExpect(status().is2xxSuccessful())
				.andExpect(MockMvcResultMatchers.content().string(containsString("Spring Boot 2.0 Projects")));
		mvc.perform(
				MockMvcRequestBuilders.delete("/book/7")
		)
				.andExpect(status().is2xxSuccessful());
		mvc.perform(
				MockMvcRequestBuilders.get("/book")
				...
				.andExpect(status().is2xxSuccessful())
				.andExpect(MockMvcResultMatchers.content().string(not(containsString("Spring Boot 2.0 Projects"))));
	}
}
